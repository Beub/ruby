...
def print_odd_indexed_integers(untableau)
	untableau.each_with_index { |item, index| puts item if index % 2 != 0 }
end

	
 # TODO: Return the odd numbers from a list of integers.
#       You should make use of Enumerable#select
def odd_integers(hello)
	hello.select { |x| x % 2 != 0 }
end
...
def sorted_triples(array)
	result_array = []
	array.each_slice(3) { |x| result_array << x.sort} 
	return result_array
end 
...
puts first_under([13, 21, 7, 0, 11, 106], 18) == [13, 7, 0, 11]
